...
| 👤 Traveller | 🛠️ Admin |
|---|---|
| **Taste** — restaurants per destination with dietary filter chips; **editable** — ✕ to remove a restaurant, and "＋ More places to eat in [city]" to fetch fresh spots for that stop. **Do** — activities by category (Culture/Nature/Adventure/Family); **editable** — ✕ to remove an activity you don't want, and "＋ More things to do in [city]" to fetch fresh ideas for that stop. **Both tabs:** every card shows a **live data badge** (★ rating · review count · price · Open now · ♿) when `GOOGLE_PLACES_API_KEY` is set — marked "Live" to distinguish verified facts from AI estimates; falls back to the **⭐ Reviews** link otherwise. And **＋ Plan** — pick a day and the place lands in that day's hour-by-hour timeline (restaurants slot in at 19:00 dinner, activities at 14:00; days you're in that city are highlighted coral). The pool→plan loop. **Nearby** — day-trip suggestions from each main destination. **Photos** — Wikipedia gallery of each destination. **Tips** — 12-15 destination-specific tips covering tipping, dress codes, photography etiquette, body language, queue norms, food, transport, health, scams. | `handleTips` uses the enriched TIPS_SYSTEM prompt. Photos from `useWikiImage`. **Do editing:** `DoTab` takes an `onUpdate` prop (wired in `ResultsView` → `onUpdateResults({ activities })`); `removeActivity()` filters by name, `fetchMore()` calls `generateActivityAlternatives()` → `/api/activityAlternatives` (`handleActivityAlternatives`, reusing the module-scope `ACTIVITIES_SYSTEM` prompt with an `exclude` list). Same edit pattern as Hotels. **Undo:** every removal here routes through `ResultsView.handleUpdate`, which detects the shrink (deep-counting the per-destination groups) and offers a one-tap Undo toast. **＋ Plan:** shared `AddToDay` picker (`src/components/shared/AddToDay.tsx`) + `addStopToDay()` (`src/lib/planEdit.ts`) — inserts a `TimelineEvent` sorted by time, or appends to `activities` on no-timeline days; wired via `onUpdateItinerary` → `onUpdateResults({ itinerary })`. **Live data:** `PlaceRating` (`src/components/shared/PlaceRating.tsx`) self-fetches via `placeCache.ts` → `/api/placeDetails` (`handlePlaceDetails`, Google Places API New, key-optional); renders nothing when unavailable so the `reviewsUrl()` link fallback stays. Cache dedupes by query + persists to sessionStorage; one "no-key" result short-circuits all further calls. |
...
- **One choke point.** Every plan edit already flowed through `ResultsView`'s `onUpdateResults`. A new `handleUpdate` wraps it: when an update *removes* items it snapshots the prior values of just the changed keys and shows a **"Removed · Undo"** toast; one tap restores them. Non-destructive updates (reorder, "make this my pick", "＋ more options", the ＋Plan insert, retries) pass straight through with no toast.
- **Removal detection is deep.** `itemCount` counts items *inside* the grouped shapes — itinerary (activities + timeline per day), `activities` and `restaurants` (grouped by destination) — because removing one shrinks an inner array without changing the top-level length. Counting only the outer array would have registered a removal as "nothing changed". This was caught in runtime testing: the first version offered no undo on the Do/Taste tabs.
- **Covers:** remove a timeline stop or bullet activity (Itinerary), remove a restaurant (Taste), remove an activity (Do), and any chat-driven deletion — all through the same path, since ChatTab's `onUpdateResults` is now `handleUpdate` too.
- **Toast gained an optional action.** `useToast(message, tone, action?)` — an actionable toast lingers 6s (vs 2.4s) so it's reachable, and the action button is keyboard-focusable. Backward compatible: existing `toast(msg)` / `toast(msg, tone)` calls are unchanged.
- **Scope:** this covers the plan (the `results` object). Expenses, wallet items and journal photos live in separate stores and still delete immediately — a future pass could extend the same pattern there. Undo is single-level (the last removal), matching the toast's lifetime.

*Verified at runtime on a production build: removed an activity → item gone + Undo toast → tapped Undo → item restored on screen and in the saved trip. Gates: `tsc -b`, `eslint src/`, `vite build` all clean.*
