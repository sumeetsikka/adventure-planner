...
export default function HotelsTab({ hotels, destinations, config }: Props) {
  ...
  if (hotels.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-[var(--cream)] font-medium mb-1">Hotel recommendations are loading</p>
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-[var(--cream)] font-bold text-xl mb-1">Your Hotel Guide</h2>
        <p className="text-[var(--text-muted)] text-sm">Premium 4-5 star picks. Tap a destination to see options, then tap a hotel for full details.</p>
      </div>
          ...
          <div className="rounded-2xl border border-[var(--line)] bg-[var(--ink-3)] p-6">
                ...
                <div className="flex items-center gap-2 mb-1">
                  {selHotel.recommended && (
                    <span className="text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider"
                      style={{ background: 'linear-gradient(135deg, #D4A574, #D4960F)', color: '#0B1120' }}>⭐ Our Pick</span>
                  )}
                  <span className="text-[#D4A574] text-xs">{'★'.repeat(selHotel.stars)}{'☆'.repeat(5 - selHotel.stars)}</span>
                  <span className="text-[var(--text-muted)] text-xs">{selHotel.style}</span>
                </div>
                <h3 className="text-[var(--cream)] font-bold text-lg">{selHotel.name}</h3>
                <p className="text-[var(--text-muted)] text-sm">{selHotel.area}, {selDest.destination}</p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-5">
              <div className="bg-[var(--ink-2)] rounded-xl p-3">
                <p className="text-[9px] text-[var(--text-dim)] uppercase tracking-wider mb-1">Per Night</p>
                <p className="text-[#7A9082] font-bold text-lg">{selHotel.price_per_night_aud}</p>
              </div>
              <div className="bg-[var(--ink-2)] rounded-xl p-3">
                <p className="text-[9px] text-[var(--text-dim)] uppercase tracking-wider mb-1">Total Est.</p>
                <p className="text-[#7A9082] font-bold text-lg">{estimateTotal(selHotel.price_per_night_aud, selDest.nights)}</p>
              </div>
              <div className="bg-[var(--ink-2)] rounded-xl p-3 sm:col-span-1 col-span-2">
                <p className="text-[9px] text-[var(--text-dim)] uppercase tracking-wider mb-1">Stay</p>
            ...
            <div className="flex flex-wrap gap-1.5 mb-4">
              {selHotel.highlights.map((hl) => (
                <span key={hl} className="text-[11px] px-2.5 py-1 rounded-full bg-[var(--ink-2)] border border-[var(--line)] text-gray-300">{hl}</span>
              ))}
            </div>
            ...
            <p className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider mb-2">Book This Hotel</p>
      ...
      <div className="space-y-3">
        {hotels.map((dest, di) => {
          ...
          return (
            <div key={di} className="rounded-2xl border border-[var(--line)] bg-[var(--ink-3)] overflow-hidden">
              {/* Destination header (clickable) */}
              <button
                onClick={() => setExpandedDest(isExpanded ? null : di)}
                className="w-full flex items-center gap-3 px-5 py-4 text-left hover:bg-[var(--ink-4)] transition-colors"
              >
                <span className="text-2xl flex-shrink-0">{info.emoji}</span>
                <div className="flex-1 min-w-0">
                  <h3 className="text-[var(--cream)] font-bold text-sm">{dest.destination}</h3>
                  <p className="text-[var(--text-muted)] text-[11px]">
                    {formatDateAU(dest.check_in)} to {formatDateAU(dest.check_out)} · {dest.nights} night{dest.nights > 1 ? 's' : ''}
                  </p>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="text-[10px] text-[var(--text-muted)]">{dest.hotels.length} hotels</span>
                  <span className={`text-[var(--text-muted)] text-xs transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}>▾</span>
                </div>
              </button>
                      ...
                      <div key={hi}
                        ...
                        className={`group relative rounded-xl p-4 border cursor-pointer transition-all duration-300 ${
                          isSelected
                            ...
                            : 'border-[var(--line)] bg-[var(--ink-2)] hover:border-[var(--line-strong)] hover:bg-[#141E33] hover:-translate-y-0.5 hover:shadow-lg'
                        }`}>
                        {/* Our Pick badge */}
                        {h.recommended && (
                          <div className="absolute top-3 right-3 text-[8px] font-bold px-2 py-0.5 rounded-full"
                            style={{ background: 'linear-gradient(135deg, #D4A574, #D4960F)', color: '#0B1120' }}>⭐ Pick</div>
                        )}

                        {/* Stars */}
                        <p className="text-[#D4A574] text-[10px] mb-1">{'★'.repeat(h.stars)} · {h.style}</p>

                        {/* Name */}
                        <h4 className="text-[var(--cream)] font-bold text-sm mb-1 pr-14 leading-snug">{h.name}</h4>
                        <p className="text-[var(--text-dim)] text-[10px] mb-3">{h.area}</p>

                        {/* Price */}
                        <div className="flex items-baseline gap-2 mb-2">
                          <span className="text-[#7A9082] font-bold text-base">{h.price_per_night_aud}</span>
                          <span className="text-[var(--text-dim)] text-[10px]">/night</span>
                          <span className="text-[var(--text-dim)] text-[10px]">· {estimateTotal(h.price_per_night_aud, dest.nights)} total</span>
                        </div>

                        {/* Highlights preview */}
                        <div className="flex flex-wrap gap-1 mb-2">
                          {h.highlights.slice(0, 3).map((hl) => (
                            <span key={hl} className="text-[9px] px-1.5 py-0.5 rounded bg-[var(--ink-3)] text-[var(--text-muted)]">{hl}</span>
                          ))}
                        </div>
                          ...
                          {(() => {
                            ...
                            return Object.entries(links).slice(0, 3).map(([name, url]) => (
                              <a key={name} href={url} target="_blank" rel="noopener noreferrer"
                                ...
                                className="text-[9px] font-semibold px-2 py-1 rounded-lg bg-[#D4A574]/10 text-[#D4A574] hover:bg-[#D4A574]/20 transition-colors">
                                {name} ↗
                              </a>
                            ));
                          })()}
